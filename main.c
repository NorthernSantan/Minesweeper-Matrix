...
void evaluateSum (boardType *B, int row, int col, int *sum, const int maxRow, const int maxCols) {

    // check if cell exists
    if (row < 1 || row > maxRow || col < 1 || col > maxCols || B->cells[row][col].status == 'O') {
       printf ("Invalid column or row value\n");
        return;
    }

    if (*sum == 0 || B->cells[row][col].colour == green) {

        printf ("In first or green cell\n");

        //first time we run through all
        // Open cell
        ...
        if (B->cells[row][col].colour == blue)
            *sum += 2 * (B->cells[row][col].value);

        else
            *sum += B->cells[row][col].value;

        // call on cell above
        evaluateSum (B, row - 1, col, sum, maxRow, maxCols);

        // call on cell below
        evaluateSum (B, row + 1, col, sum, maxRow, maxCols);

        // call on cell left
        evaluateSum (B, row, col - 1, sum, maxRow, maxCols);

        // call on cell right
        evaluateSum (B, row, col + 1, sum, maxRow, maxCols);

        // call on cell above
        evaluateSum (B, row - 1, col - 1, sum, maxRow, maxCols);
        ...
        printf ("Done first or green\n");

    } else {

        printf ("In terminal cell (leaf)\n");
...
int main (){
  ...
  while (1){
    ...
    evaluateSum (&board, row, col, &score, maxRows, maxCols);
